...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        image = (
            plt.imread(self.base_path + "/" + self.imnames[item][0]).transpose(2, 0, 1)
            / 255
        )
        weight, target = self.generate_target(image.shape, self.charBB[item].copy())
        weight_affinity, target_affinity = self.generate_affinity(
            image.shape, self.charBB[item].copy(), self.txt[item].copy()
        )
