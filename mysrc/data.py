...
class ParamDataset(BaseDataset):
        ...
        def save(bk, im, bbox=None, pre_cat="None", next_cat="None"):
            if "Blend" in bk["cat"]:
                ...
                for i in range(len(_bks)):
                    ...
                    next_cat = _bks[i +
                                    1]["cat"] if i < len(_bks) - 1 else "None"
    ...
    def __getitem__(self, index):
        ...
        xy0 = np.array(bbox[:2])
        xy1 = np.array(bbox[-2:])
        xy = xy0 + np.random.randint(-5, 5, 2)  # 不需clip，轉成mask後自動會clip
        wh = np.clip(xy1 - xy0 + np.random.randint(-5, 5, 2),
                     5, 10000000)
...
class ParamTripletDataset(BaseDataset):
    param_filter = (
        ("_rgb", np.zeros(3)),
        ("_a", np.zeros(1)),
        ("_textsize", np.zeros(1)),
